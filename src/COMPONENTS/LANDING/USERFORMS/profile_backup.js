      ...
      fetch(API_URL + "/users/resend-email", {
        ...
        .then((responseData) => {
          if (responseData.status == 200) {
            ...
            this.setState({ loader: false })
            this.openNotificationWithIcon('warning', responseData.err, 'Please check it by logging in.')
          }
        })
        .catch(error => {
          this.setState({ loader: false })
      ...
      fetch(API_URL + "/users/forgot-twofactors", {
        ...
        .then((responseData) => {
          if (responseData.status == 200) {
            ...
            this.setState({ loader: false })
            this.openNotificationWithIcon('warning', 'Warning', responseData.err)
          }
        })
        .catch(error => {
          this.setState({ loader: false })
          ...
          <ColRight sm={24} lg={12}>
            <FormWrap  >
              <RightWrap className="wow fadeInDown" >
                ...
                <SubText>Please upload your photo of less than 10 mb and you will be notified by our team through email.</SubText>
                <FileWrapDup>
                  <Fileselect1 className="file-select-col">
                    ...
                    <ButtonUpDup style={{ backgroundImage: `url('${this.state.profileImg}')` }} className="file-select-btn" onClick={() => { this.handleFileSelectClick('front') }}>

                      <PlusDup className="plus"><Icon type={this.state.icon1} theme="outlined" /></PlusDup>
                      <PlustextDup className="text">Upload</PlustextDup>
                    </ButtonUpDup>
                    <Fileinput onChange={this.handleProfile} type="file" name="front-doc" id="front" />
                  </Fileselect1>
                </FileWrapDup>
                <ButtonLogin onClick={this.submitProfile} disabled={!this.state.flagImage}>SUBMIT</ButtonLogin>
