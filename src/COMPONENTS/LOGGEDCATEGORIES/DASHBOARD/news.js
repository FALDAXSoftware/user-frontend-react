...
class News extends Component {
  ...
  loadNews = (page) => {
    ...
    fetch(
      `${globalVariables.API_URL}/users/get-all-news?limit=50&page=${page}`,
      {
        method: "post",
        headers: {
  ...
  render() {
    return (
      <>
        <NewsHeader>{this.t("footer:subhead_news.message")}</NewsHeader>
        <NewsList>
          <Scrollbars
            // style={{ height: 560 }}
              ...
              <List>
                <Date>
                  {moment
                    ...
                    .format(`${this.props.profileDetails.date_format} HH:mm`)}
                </Date>
                <ListSpan>
...
export default translate(["footer"])(connect(mapStateToProps)(News));
